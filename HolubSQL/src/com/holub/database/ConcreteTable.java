...
/* package */ class ConcreteTable implements Table {
	...
	public final static class Test {
		...
		public void testStoreHTML() throws IOException, ClassNotFoundException { // Flush the table to disk, then reread
																					// it.
			// Subsequent tests that use the
			// "people" table will
			// fail if this operation fails.

			Writer out = new FileWriter("people_HTML");
			people.export(new HTMLExporter(out));
			out.close();

			// Reader in = new FileReader("people_HTML");
			// people = new ConcreteTable(new CSVImporter(in));
			// in.close();
		}
